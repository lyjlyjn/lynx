...
#include "../include/white_noise.h" /* <-- added: white noise declarations */

/* 新增：Todo 菜单 ID（如项目有集中管理，请合并到对应 header） */
...
/** White noise runtime globals (defined elsewhere, e.g., src/timer.c or dedicated white_noise module) */
...
void ShowColorMenu(HWND hwnd) {
    ...
    for (int i = 0; i < CLOCK_RECENT_FILES_COUNT; i++) {
        ...
        BOOL isCurrentFile = (CLOCK_TIMEOUT_ACTION == TIMEOUT_ACTION_OPEN_FILE && 
                             strlen(CLOCK_RECENT_FILES[i].path) > 0 && 
    ...
    /* ===== 插入待办菜单项（右键菜单） ===== */
    AppendMenuW(hMenu, MF_STRING, CLOCK_IDM_TODO, GetLocalizedString(L"待办", L"Todo"));
    /* ===== 插入结束 ===== */
...
void ShowContextMenu(HWND hwnd) {
     ...
     * 构建白噪音菜单
     */
    HMENU hWhiteNoiseMenu = CreatePopupMenu();
    
    /* 检查白噪音是否正在播放 */
    BOOL isPlaying = WhiteNoise_IsPlaying();
    
    const wchar_t* playPauseText = isPlaying ?
        GetLocalizedString(L"暂停", L"Pause") :
        GetLocalizedString(L"播放", L"Play");
    
    AppendMenuW(hWhiteNoiseMenu, MF_STRING,
               CLOCK_IDM_WHITE_NOISE_TOGGLE, playPauseText);
    
    AppendMenuW(hWhiteNoiseMenu, MF_SEPARATOR, 0, NULL);
    
    /* 获取当前类型 */
    WhiteNoiseType currentType = WhiteNoise_GetCurrentType();
    
    /* 添加5种白噪音选项 */
    AppendMenuW(hWhiteNoiseMenu, MF_STRING | (currentType == WHITE_NOISE_RAIN ? MF_CHECKED : 0),
               CLOCK_IDM_WHITE_NOISE_RAIN,
               GetLocalizedString(L"🌧️ 雨声", L"🌧️ Rain"));
    
    AppendMenuW(hWhiteNoiseMenu, MF_STRING | (currentType == WHITE_NOISE_WIND ? MF_CHECKED : 0),
               CLOCK_IDM_WHITE_NOISE_WIND,
               GetLocalizedString(L"💨 风声", L"💨 Wind"));
    
    AppendMenuW(hWhiteNoiseMenu, MF_STRING | (currentType == WHITE_NOISE_COFFEE ? MF_CHECKED : 0),
               CLOCK_IDM_WHITE_NOISE_COFFEE,
               GetLocalizedString(L"☕ 咖啡厅", L"☕ Coffee Shop"));
    
    AppendMenuW(hWhiteNoiseMenu, MF_STRING | (currentType == WHITE_NOISE_FOREST ? MF_CHECKED : 0),
               CLOCK_IDM_WHITE_NOISE_FOREST,
               GetLocalizedString(L"🌲 森林", L"🌲 Forest"));
    
    AppendMenuW(hWhiteNoiseMenu, MF_STRING | (currentType == WHITE_NOISE_OCEAN ? MF_CHECKED : 0),
               CLOCK_IDM_WHITE_NOISE_OCEAN,
               GetLocalizedString(L"🌊 海浪", L"🌊 Ocean"));
    
    AppendMenuW(hWhiteNoiseMenu, MF_SEPARATOR, 0, NULL);
    
    /* 音量控制 */
    wchar_t volumeText[64];
    swprintf_s(volumeText, sizeof(volumeText) / sizeof(wchar_t),
        GetLocalizedString(L"音量: %d%%", L"Volume: %d%%"),
        CLOCK_WHITE_NOISE_VOLUME);
    
    AppendMenuW(hWhiteNoiseMenu, MF_STRING, CLOCK_IDM_WHITE_NOISE_VOLUME_PLUS,
               GetLocalizedString(L"🔊 音量 +", L"🔊 Volume +"));
    
    AppendMenuW(hWhiteNoiseMenu, MF_STRING, CLOCK_IDM_WHITE_NOISE_VOLUME_MINUS,
               GetLocalizedString(L"🔇 音量 -", L"🔇 Volume -"));
    
    AppendMenuW(hWhiteNoiseMenu, MF_SEPARATOR, 0, NULL);
    
    AppendMenuW(hWhiteNoiseMenu, MF_STRING, CLOCK_IDM_WHITE_NOISE_STOP,
               GetLocalizedString(L"停止", L"Stop"));
    ...
    /* 添加白噪音菜单 */
    AppendMenuW(hMenu, MF_POPUP, (UINT_PTR)hWhiteNoiseMenu,
               GetLocalizedString(L"🎵 白噪音", L"🎵 White Noise"));
    ...
    GetConfigPath(configPath2, MAX_PATH);
    wchar_t wconfigPath2[MAX_PATH];
    MultiByteToWideChar(CP_UTF8, 0, configPath2, -1, wconfigPath2, MAX_PATH);
    
    FILE *configFile2 = _wfopen(wconfigPath2, L"r");
    POMODORO_TIMES_COUNT = 0;
    
    if (configFile2) {
        char line[256];
        while (fgets(line, sizeof(line), configFile2)) {
    ...
    HMENU hPomodoroMenu2 = CreatePopupMenu();
    
    wchar_t timeBuffer2[64];
    
    AppendMenuW(hPomodoroMenu2, MF_STRING, CLOCK_IDM_POMODORO_START,
                GetLocalizedString(L"开始", L"Start"));
    AppendMenuW(hPomodoroMenu2, MF_SEPARATOR, 0, NULL);

    /** Generate dynamic Pomodoro time options with current phase indication */
    for (int i = 0; i < POMODORO_TIMES_COUNT; i++) {
        FormatPomodoroTime(POMODORO_TIMES[i], timeBuffer2, sizeof(timeBuffer2)/sizeof(wchar_t));
        ...
        AppendMenuW(hPomodoroMenu2, MF_STRING | (isCurrentPhase ? MF_CHECKED : MF_UNCHECKED), 
                    menuId, timeBuffer2);
    }
    ...
    _snwprintf(menuText, sizeof(menuText)/sizeof(wchar_t),
              ...
              POMODORO_LOOP_COUNT);
    AppendMenuW(hPomodoroMenu2, MF_STRING, CLOCK_IDM_POMODORO_LOOP_COUNT, menuText);

    AppendMenuW(hPomodoroMenu2, MF_SEPARATOR, 0, NULL);

    AppendMenuW(hPomodoroMenu2, MF_STRING, CLOCK_IDM_POMODORO_COMBINATION,
              GetLocalizedString(L"组合", L"Combination"));
    
    AppendMenuW(hMenu, MF_POPUP, (UINT_PTR)hPomodoroMenu2,
