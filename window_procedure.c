...
#include "../include/white_noise.h"  
#include "../include/white_noise.h"

// === Todo list embedded controls & helpers (added) ===
#define IDC_TODO_LIST 60001
#define IDC_TODO_INPUT 60002
#define IDC_TODO_ADD 60003
#define IDC_TODO_DEL 60004
#define IDC_TODO_TOGGLE 60005

HWND hTodoList = NULL;
HWND hTodoInput = NULL;
HWND hTodoAddBtn = NULL;
HWND hTodoDelBtn = NULL;
HWND hTodoToggleBtn = NULL;
HFONT hTodoFont = NULL;
COLORREF TODO_BG_COLOR = RGB(245,245,245);

void GetTodoFilePath(wchar_t *path, size_t size) {
    wchar_t appData[MAX_PATH] = {0};
    if (SHGetFolderPathW(NULL, CSIDL_LOCAL_APPDATA, NULL, 0, appData) != S_OK) {
        path[0] = L'\0';
        return;
    }
    _snwprintf(path, size, L"%s\\Catime\\todo.txt", appData);
}

void LoadTodoItems(HWND listBox) {
    if (!listBox) return;
    wchar_t path[MAX_PATH] = {0};
    GetTodoFilePath(path, MAX_PATH);
    ...
    while (fgetws(line, 512, f)) {
        ...
        while (ln > 0 && (line[ln-1] == L'\n' || line[ln-1] == L'\r')) { line[ln-1] = L'\0'; ln--; }
        if (ln > 0) SendMessageW(listBox, LB_ADDSTRING, 0, (LPARAM)line);
    }
    fclose(f);
}

void SaveTodoItems(HWND listBox) {
    if (!listBox) return;
    wchar_t path[MAX_PATH] = {0};
    GetTodoFilePath(path, MAX_PATH);
    if (path[0] == L'\0') return;
    wchar_t dir[MAX_PATH]; wcscpy(dir, path);
    wchar_t *p = wcsrchr(dir, L'\\');
    if (p) { *p = L'\0'; SHCreateDirectoryExW(NULL, dir, NULL); }
    FILE *f = _wfopen(path, L"w, ccs=UTF-8");
    if (!f) return;
    int count = (int)SendMessageW(listBox, LB_GETCOUNT, 0, 0);
    for (int i = 0; i < count; ++i) {
        wchar_t buf[512] = {0};
        SendMessageW(listBox, LB_GETTEXT, i, (LPARAM)buf);
        if (wcslen(buf) > 0) {
            fputws(buf, f);
            fputwc(L'\n', f);
        }
...
HWND CreateSimpleButton(HWND parent, LPCWSTR text, int x, int y, int w, int h, int id, HINSTANCE hInst) {
    HWND btn = CreateWindowW(L"BUTTON", text, WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                             x, y, w, h, parent, (HMENU)(INT_PTR)id, hInst, NULL);
    if (hTodoFont) SendMessageW(btn, WM_SETFONT, (WPARAM)hTodoFont, TRUE);
    return btn;
}
// === end todo helpers ===
...
#ifndef CLOCK_IDM_TODO
#define CLOCK_IDM_TODO 40001
#endif
...
/* White Noise Control */
extern BOOL CLOCK_ENABLE_WHITE_NOISE;
extern int CLOCK_WHITE_NOISE_TYPE;
extern int CLOCK_WHITE_NOISE_VOLUME;
...
WhiteNoise_Init();
    ...
    // === Embedded Todo UI creation (injected) ===
    hTodoFont = CreateFontW(-14,0,0,0,FW_NORMAL,FALSE,FALSE,FALSE,
                            DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                            CLEARTYPE_QUALITY, DEFAULT_PITCH|FF_SWISS, L"Segoe UI");

    HWND hTodoBg = CreateWindowW(L"STATIC", NULL,
        WS_CHILD | WS_VISIBLE | SS_NOTIFY,
        20, 220, 560, 160, hwnd, NULL, GetModuleHandle(NULL), NULL);
    SetClassLongPtrW(hTodoBg, GCLP_HBRBACKGROUND, (LONG_PTR)CreateSolidBrush(TODO_BG_COLOR));

    hTodoList = CreateWindowW(L"LISTBOX", NULL,
        WS_CHILD | WS_VISIBLE | WS_BORDER | LBS_NOTIFY | WS_VSCROLL,
        40, 240, 520, 100, hwnd, (HMENU)IDC_TODO_LIST, GetModuleHandle(NULL), NULL);

    hTodoInput = CreateWindowW(L"EDIT", L"",
        WS_CHILD | WS_VISIBLE | WS_BORDER,
        40, 350, 340, 24, hwnd, (HMENU)IDC_TODO_INPUT, GetModuleHandle(NULL), NULL);

    hTodoAddBtn = CreateSimpleButton(hwnd, L"添加", 400, 350, 60, 24, IDC_TODO_ADD, GetModuleHandle(NULL));
    hTodoDelBtn = CreateSimpleButton(hwnd, L"删除", 468, 350, 60, 24, IDC_TODO_DEL, GetModuleHandle(NULL));
    hTodoToggleBtn = CreateSimpleButton(hwnd, L"完成", 536, 350, 60, 24, IDC_TODO_TOGGLE, GetModuleHandle(NULL));

    if (hTodoList) SendMessageW(hTodoList, WM_SETFONT, (WPARAM)hTodoFont, TRUE);
    if (hTodoInput) SendMessageW(hTodoInput, WM_SETFONT, (WPARAM)hTodoFont, TRUE);

    // Load persisted todo items
    LoadTodoItems(hTodoList);
break;
        }
...
WhiteNoise_Cleanup();
            ...
            ConfigWatcher_Stop();
            // Persist todo items before exit
            SaveTodoItems(hTodoList);
    ...
    // === Embedded Todo command handling (injected) ===
    if (cmd == IDC_TODO_ADD) {
        wchar_t buf[512] = {0};
        GetWindowTextW(hTodoInput, buf, 512);
        if (wcslen(buf) > 0) {
            SendMessageW(hTodoList, LB_ADDSTRING, 0, (LPARAM)buf);
            SetWindowTextW(hTodoInput, L"");
            SaveTodoItems(hTodoList);
        }
        return 0;
    } else if (cmd == IDC_TODO_DEL) {
        int sel = (int)SendMessageW(hTodoList, LB_GETCURSEL, 0, 0);
        if (sel != LB_ERR) {
            SendMessageW(hTodoList, LB_DELETESTRING, sel, 0);
            SaveTodoItems(hTodoList);
        }
        return 0;
    } else if (cmd == IDC_TODO_TOGGLE) {
        int sel = (int)SendMessageW(hTodoList, LB_GETCURSEL, 0, 0);
        if (sel != LB_ERR) {
            wchar_t buf[512] = {0};
            SendMessageW(hTodoList, LB_GETTEXT, sel, (LPARAM)buf);
            if (wcsstr(buf, L"[完成]") == NULL) wcsncpy(buf + wcslen(buf), L" [完成]", 512 - wcslen(buf));
            else {
                wchar_t *p = wcsstr(buf, L" [完成]");
                if (p) *p = L'\0';
            }
            SendMessageW(hTodoList, LB_DELETESTRING, sel, 0);
            SendMessageW(hTodoList, LB_INSERTSTRING, sel, (LPARAM)buf);
            SaveTodoItems(hTodoList);
        }
        return 0;
    }
...
case CLOCK_IDM_WHITE_NOISE_TOGGLE: {
    // 调试日志
    char log_msg[256];
    sprintf_s(log_msg, 256, "Toggle clicked.\nIs_playing=%d\nType=%d\nVolume=%d",
        WhiteNoise_IsPlaying(), CLOCK_WHITE_NOISE_TYPE, CLOCK_WHITE_NOISE_VOLUME);
    MessageBoxA(hwnd, log_msg, "Debug - Toggle", MB_OK);
    
    if (WhiteNoise_IsPlaying()) {
        WhiteNoise_Stop();
        CLOCK_ENABLE_WHITE_NOISE = FALSE;
        MessageBoxW(hwnd, L"白噪音已停止", L"Info", MB_OK);
    } else {
        BOOL result = WhiteNoise_Play((WhiteNoiseType)CLOCK_WHITE_NOISE_TYPE, 
                        CLOCK_WHITE_NOISE_VOLUME);
        if (result) {
            CLOCK_ENABLE_WHITE_NOISE = TRUE;
            MessageBoxW(hwnd, L"播放成功！", L"Success", MB_OK);
        } else {
            MessageBoxW(hwnd, 
                L"播放失败\n\n可能原因：\n1. 音频文件不存在\n2. 文件格式错误\n3. 音频设备问题", 
...
case CLOCK_IDM_WHITE_NOISE_RAIN:
case CLOCK_IDM_WHITE_NOISE_WIND:
case CLOCK_IDM_WHITE_NOISE_COFFEE:
case CLOCK_IDM_WHITE_NOISE_FOREST:
case CLOCK_IDM_WHITE_NOISE_OCEAN: {
    WhiteNoiseType type;
    const wchar_t* type_name = L"";
    
    switch (LOWORD(wp)) {
        case CLOCK_IDM_WHITE_NOISE_RAIN:
            type = WHITE_NOISE_RAIN;
            CLOCK_WHITE_NOISE_TYPE = 0;
            type_name = L"雨声";
            break;
        case CLOCK_IDM_WHITE_NOISE_WIND:
            type = WHITE_NOISE_WIND;
            CLOCK_WHITE_NOISE_TYPE = 1;
            type_name = L"风声";
            break;
        case CLOCK_IDM_WHITE_NOISE_COFFEE:
            type = WHITE_NOISE_COFFEE;
            CLOCK_WHITE_NOISE_TYPE = 2;
            type_name = L"咖啡厅";
            break;
        case CLOCK_IDM_WHITE_NOISE_FOREST:
            type = WHITE_NOISE_FOREST;
            CLOCK_WHITE_NOISE_TYPE = 3;
            type_name = L"森林";
            break;
        case CLOCK_IDM_WHITE_NOISE_OCEAN:
            type = WHITE_NOISE_OCEAN;
            CLOCK_WHITE_NOISE_TYPE = 4;
            type_name = L"海浪";
            break;
        default:
            type = WHITE_NOISE_RAIN;
    ...
    swprintf_s(debug_msg, 256, L"选择类型: %s\nType=%d\nVolume=%d", 
        type_name, type, CLOCK_WHITE_NOISE_VOLUME);
    MessageBoxW(hwnd, debug_msg, L"Debug - Type", MB_OK);
    
    BOOL result = WhiteNoise_Play(type, CLOCK_WHITE_NOISE_VOLUME);
    if (result) {
        CLOCK_ENABLE_WHITE_NOISE = TRUE;
        
        wchar_t success_msg[256];
        swprintf_s(success_msg, 256, L"%s 播放成功！", type_name);
        MessageBoxW(hwnd, success_msg, L"Success", MB_OK);
    } else {
        wchar_t error_msg[256];
        swprintf_s(error_msg, 256, L"%s 播放失败\n请检查音频文件", type_name);
        MessageBoxW(hwnd, error_msg, L"Error", MB_ICONERROR);
    }
...
case CLOCK_IDM_WHITE_NOISE_VOLUME_PLUS: {
    int old_volume = CLOCK_WHITE_NOISE_VOLUME;
    CLOCK_WHITE_NOISE_VOLUME += 10;
    if (CLOCK_WHITE_NOISE_VOLUME > 100) {
        CLOCK_WHITE_NOISE_VOLUME = 100;
    }
    
    WhiteNoise_SetVolume(CLOCK_WHITE_NOISE_VOLUME);
    
    // 调试信息
    wchar_t msg[128];
    swprintf_s(msg, 128, L"音量: %d%% → %d%%", old_volume, CLOCK_WHITE_NOISE_VOLUME);
    MessageBoxW(hwnd, msg, L"Volume +", MB_OK);
...
case CLOCK_IDM_WHITE_NOISE_VOLUME_MINUS: {
    int old_volume = CLOCK_WHITE_NOISE_VOLUME;
    CLOCK_WHITE_NOISE_VOLUME -= 10;
    if (CLOCK_WHITE_NOISE_VOLUME < 0) {
        CLOCK_WHITE_NOISE_VOLUME = 0;
    }
...
case CLOCK_IDM_WHITE_NOISE_STOP: {
    WhiteNoise_Stop();
    CLOCK_ENABLE_WHITE_NOISE = FALSE;
    
    MessageBoxW(hwnd, L"白噪音已停止", L"Stop", MB_OK);
...
case CLOCK_IDM_TODO: {
    LOG_INFO("[WindowProc] CLOCK_IDM_TODO selected - opening Todo dialog");
    Todo_ShowDialog(hwnd);
    break;
}
                case CLOCK_IDM_HELP: {
                    OpenUserGuide();
